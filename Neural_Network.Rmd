...
best1 <- which.min(nnmodel$result.matrix["error", ])
pred_clannts <- predict(nnmodel, tsdata, rep = best1)
```
```{r}
...
# Sensitivity and Specificity via caret
cm <- caret::confusionMatrix(factor(pred_classnnts, levels = c(0, 1)),
                             factor(tsdata$Admit, levels = c(0, 1)),
                             positive = "1")
...
best2 <- which.min(nnmodel2$result.matrix["error", ])
pred_clannts2 <- predict(nnmodel2, tsdata, rep = best2)
```
...
conf_clannts2  # confusion Matrix from nn model
...
# Sensitivity and Specificity via caret
cm2 <- caret::confusionMatrix(factor(pred_classnnts2, levels = c(0, 1)),
                              factor(tsdata$Admit, levels = c(0, 1)),
                              positive = "1")
...
```
  ...
  Overall Accuracy .8625

  Misclassification .1375
...
nnmodel3 <- neuralnet(Admit~ CGPA+GRE+SOP,
                     ...
                     linear.output = FALSE)  # Neural Network Model
# Predicting test data.
best3 <- which.min(nnmodel3$result.matrix["error", ])
pred_clannts3 <- predict(nnmodel3, tsdata, rep = best3)
```
...
conf_clannts3  # confusion Matrix from nn model
...
# Sensitivity and Specificity via caret
cm3 <- caret::confusionMatrix(factor(pred_classnnts3, levels = c(0, 1)),
                              factor(tsdata$Admit, levels = c(0, 1)),
                              positive = "1")
...
```
  ...
  Overall Accuracy .8375

  Misclassification .1625
...
# 9. The same data was used to predict Admit in Compare the test misclassification error from the decision tree, Random Forest, Random Forest-Bagging, the logistic model and neural network model. Which model is best? 
  ...
  Random Forest-Bagging .1375

  Logistic Model .1125

  Neural Network (2 hidden units) .1375
